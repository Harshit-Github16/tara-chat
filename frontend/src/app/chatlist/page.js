...
export default function ChatListPage() {
    ...
    const sendMoodGreeting = async () => {
      ...
      if (fromMood === 'true' && moodDataParam) {
        try {
          ...
          // Save to database
          try {
            await api.post('/api/user/conversations', {
              userId: user.uid,
              chatUserId: 'tara-ai',
  ...
  const loadCustomUsers = async () => {
    ...
    try {
      const response = await api.get(`/api/user?userId=${user.uid}`);
      const data = await response.json();
  ...
  const loadConversations = async (chatUserId) => {
    ...
    try {
      const response = await api.get(`/api/user/conversations?userId=${user.uid}&chatUserId=${chatUserId}`);
      const data = await response.json();
  ...
  const deleteChat = async (chatId, e) => {
    ...
    try {
      ...
      if (user?.uid) {
        const response = await fetch(`${API_BASE_URL}/api/users?userId=${user.uid}&chatUserId=${chatId}`, {
          method: 'DELETE'
        });
