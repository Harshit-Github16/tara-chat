...
export default function BlogPostClient() {
    ...
    const incrementView = async (blogId) => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/blog/${blogId}/view`, { method: 'POST' });
            const data = await response.json();
            if (data.success) {
    ...
    const loadComments = async (blogId) => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/blog/${blogId}/comment`);
            const data = await response.json();
    ...
    const handleLike = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/blog/${post.id}/like`, {
                method: 'POST',
    ...
    const handleCommentSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/blog/${post.id}/comment`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: 'anonymous',
