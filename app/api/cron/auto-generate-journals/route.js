...
export async function GET(request) {
    try {
        ...
        console.log('🕐 Starting daily journal auto-generation...');
        ...
        console.log(`👥 Found ${allUsers.length} users with chat activity`);
        ...
        for (const user of allUsers) {
            try {
                ...
                if (existingJournal) {
                    skippedCount++;
                    console.log(`⏭️  Skipped user ${user.firebaseUid} - journal already exists`);
                    continue;
                }
                ...
                if (dayMessages.length === 0) {
                    skippedCount++;
                    console.log(`⏭️  Skipped user ${user.firebaseUid} - no messages on ${targetDate}`);
                    continue;
                }
                ...
                generatedCount++;
                console.log(`✅ Generated journal for user ${user.firebaseUid} (${dayMessages.length} messages)`);
        ...
        console.log('✨ Daily journal generation complete:', result);
